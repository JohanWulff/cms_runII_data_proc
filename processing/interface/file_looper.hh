...
private:
	...
	using LorentzVectorPEPM = ROOT::Math::LorentzVector<ROOT::Math::PtEtaPhiM4D<float>>;
    ...
    unsigned long long int _get_strat_key(const int& sample, const int& jet_cat, const int pairType, const Year& year, const int& region);
    std::vector<std::string> _get_evt_names(const std::map<unsigned long, std::string>&, const std::vector<unsigned long>&);
    int _jet_cat_lookup(const bool has_b_pair, const bool has_vbf_pair, const bool isBoosted, const int num_btag_loose, const int num_btag_medium);
    ...
    bool _apply_baseline(int c_event, int pairType, int nleps, int nbjetscand, int isLeptrigger);
    int _get_region(int pairType, int isOS, float dau1_deepTauVSJet, float dau2_deepTauVSJet, float dau1_iso, float dau1_eleMVAiso ); 
public:
    // Methods
	...
	~FileLooper();
    bool loop_file(const std::string& fname, const std::string& out_dir,
